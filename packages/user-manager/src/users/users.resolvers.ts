...
import { Args, Mutation, Query, Resolver, Context, ResolveReference, ResolveField, Parent } from '@nestjs/graphql';
...
export class UserResolver {
  ...
  async forgotPassword(
    ...
    return await this.usersService.forgotPassword(email, { authorization });
  }
  ...
  async createUser(
    ...
    try {
      ...
      const { authorization } = request.headers;
      //const { authorization } = request.http;
  ...
  @ResolveField('picture_url')
  displayImages(@Parent() user: User) {
    this.logger.http("ResolveField::picture_url::FileResolver" + user.id)
    return { __typename: 'File', id: user.id };
  }
}
