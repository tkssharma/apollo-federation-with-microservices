...
import { ConfigData, ConfigDBData, SendGridConfig, AuthConfig, ApiConfig } from './config.interface';
...
export class ConfigService {
  ...
  private parseConfigFromEnv(env: NodeJS.ProcessEnv): ConfigData {
    return {
      ...
      apis: this.parseApiConfig(env)
    };
  }
  private parseApiConfig(env: NodeJS.ProcessEnv): ApiConfig {
    return {
      notificationApiUrl: env.NOTIFICATION_API_URL || '',
    };
  }
