...
export default class FileDaoService {
  constructor(
    @InjectRepository(UserUploads) private readonly userUploadRepo: Repository<UserUploads>,
    private readonly logger: Logger,
    private readonly s3: AWSS3Service

  ) { }

  async create(payload: Partial<UserUploads>): Promise<UserUploads> {
    try {
  ...
  async getByReferenceIdId(referenceId: string): Promise<UserUploads[]> {
